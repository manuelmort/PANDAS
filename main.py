...
# *** MODIFIED: collate now returns None for empty batches — below we skip them ***
...
scheduler = torch.optim.lr_scheduler.MultiStepLR(
    optimizer, milestones=[20, 100], gamma=0.1
)
...
for epoch in range(num_epochs):
    ...
    print('\n=>Epoches %i, learning rate = %.7f' %
          (epoch + 1, current_lr) +
          (', previous best = %.4f' % best_pred if (best_pred is not None) else '')
          )

    if train:
        for i_batch, sample_batched in enumerate(dataloader_train):

            # *** MODIFICATION: skip None batches ***
    ...
    if epoch % 1 == 0:
        with torch.no_grad():
            ...
            for i_batch, sample_batched in enumerate(dataloader_val):
                ...
                preds, labels, _ = evaluator.eval_test(
                    sample_batched, model, graphcam, n_features=args.n_features
                )
            ...
            if (best_pred is None) or (val_acc > best_pred):
                ...
                if not test:
                    ...
                    torch.save(
                        model.state_dict(),
                        os.path.join(model_path, task_name + ".pth")
                    )

            log = 'epoch [{}/{}] ------ acc: train = {:.4f}, val = {:.4f}\n'.format(
            ...
            writer.add_scalars(
                'accuracy',
                {
                    'train acc': trainer.get_scores(),
                    'val acc': evaluator.get_scores()
                },
                epoch + 1
            )
